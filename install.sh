...
SKIP_PLUGINS=0; SKIP_CODEX=0; SKIP_PYRIGHT=0
...
# ------------------------------------------------------------------ scripts
say "Scripts"
install -m 0755 "$REPO/claude/scripts/"*.sh "$SCRIPTS/"
install -m 0644 "$REPO/manifest.sh" "$SCRIPTS/harness-manifest.sh"
ok "installed $(ls "$REPO/claude/scripts" | wc -l) scripts + harness-manifest.sh"
